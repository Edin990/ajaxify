...
// script and style handling settings, prefetch
	...
	cachetimeout: 5000, // time in miliseconds for pages to be cached, per individual page, when memoryoff is set to true
...
class Pages { constructor() {
    ...
    this.a = function (h) {
		if (typeof h === "string" && d[h]) { 
			if(h === "f") d = {};
			else if (d[h] && !(!$.memory(h) && (Date.now() - d[h].time) >= $.s.cachetimeout)) return d[h][1]; // load cached page if fresh or memoryoff=false
		}

		if (typeof h === "object") { 
			h.time = Date.now(); //remember when page is cached
			d[h[0]] = h; //store page in cache
		}
